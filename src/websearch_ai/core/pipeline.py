...
class WebSearchPipeline:
    ...
    def __init__(self, settings: Settings):
        ...
        self.cache = (
            CacheManager(settings.cache_dir) if settings.cache_enabled else None
        )
    ...
    async def _generate_better_queries(self, query: str) -> list[str]:
        ...
        result = await self.llm.call_structured(
            prompt, BetterQueries, max_tokens=self.settings.llm_tokens_better_queries
        )
    ...
    async def _perform_searches(self, queries: list[str]) -> list[SearchResult]:
        ...
        for query in queries:
            results = self.search_engine.search(
                query, self.settings.search_max_results_per_query
            )
    ...
    async def _calculate_relevance(
        self, result: SearchResult, original_query: str
    ) -> SearchResult:
        ...
        relevance_str = await self.llm.call_text(
            prompt, max_tokens=self.settings.llm_tokens_relevance_check
        )
        ...
        try:
            ...
            logger.warning(
                f"Invalid relevance score '{relevance_str}' for {result.url}"
            )
            result.relevance = 0
    ...
    async def _filter_by_relevance(
        ...
        tasks = [self._calculate_relevance(r, original_query) for r in results]
    ...
    async def _fetch_content(self, results: list[SearchResult]) -> list[str | None]:
        ...
        async with aiohttp.ClientSession(headers=self.fetcher.headers) as session:
            ...
            for i, result in enumerate(results):
                ...
                tasks.append(
                    self.fetcher.fetch_with_cache(
                        result.url, session, self.cache, self.robots_checker, proxy
                    )
                )
    ...
    async def _summarize_content(
        ...
        prompt = self.prompt_manager.format_prompt(
            prompt_template, query=original_query, content=truncated
        )

        summary = await self.llm.call_text(
            prompt, max_tokens=self.settings.llm_tokens_summarize
        )
    ...
    async def _merge_summaries(
        self, summaries: list[SearchResult], original_query: str
    ) -> str:
        ...
        prompt = self.prompt_manager.format_prompt(
            prompt_template, query=original_query, summaries=summaries_text
        )

        logger.info("Merging summaries into final answer...")
        final_answer = await self.llm.call_text(
            prompt, max_tokens=self.settings.llm_tokens_merge
        )
        return final_answer
    ...
    async def run(self, query: str) -> tuple[list[SearchResult], str]:
        ...
        summaries = await self._generate_summaries(filtered_results, contents, query)
