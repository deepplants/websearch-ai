...
from websearch import Settings, WebSearchPipeline, SearchResult, BetterQueries
...
class TestWebSearchPipelineIntegration:
    ...
    def mock_llm_client(self):
        ...
        with patch('websearch.core.pipeline.LLMClient') as mock:
            client = Mock()
            client.call_structured = AsyncMock(return_value=BetterQueries(
    ...
    async def test_generate_better_queries(self, mock_settings, mock_llm_client):
        ...
        with patch.object(pipeline.llm, 'call_structured', 
    ...
    async def test_url_filtering_in_search(self, mock_settings):
        ...
        with patch.object(pipeline.search_engine, 'search', return_value=[
