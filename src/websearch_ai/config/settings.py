...
class Settings(BaseModel):
    ...
    openai_temperature: float = Field(
        default=0.2, ge=0, le=2, description="Temperature for LLM"
    )
    ...
    def from_yaml(cls, config_path: Path | str | None = None) -> "Settings":
        ...
        if config_path.exists():
            ...
            config_data = {
                ...
                "openai_model": yaml_data.get("openai", {}).get(
                    "model", "gpt-4.1-nano"
                ),
                "openai_temperature": yaml_data.get("openai", {}).get(
                    "temperature", 0.2
                ),
                # LLM tokens
                "llm_tokens_better_queries": yaml_data.get("llm_tokens", {}).get(
                    "better_queries", 512
                ),
                "llm_tokens_relevance_check": yaml_data.get("llm_tokens", {}).get(
                    "relevance_check", 100
                ),
                "llm_tokens_summarize": yaml_data.get("llm_tokens", {}).get(
                    "summarize_content", 2048
                ),
                "llm_tokens_merge": yaml_data.get("llm_tokens", {}).get(
                    "merge_summaries", 4096
                ),
                "llm_tokens_coverage": yaml_data.get("llm_tokens", {}).get(
                    "coverage_check", 1024
                ),
                # Search settings
                "search_max_results_per_query": yaml_data.get("search", {}).get(
                    "max_results_per_query", 5
                ),
                "search_total_max_results": yaml_data.get("search", {}).get(
                    "total_max_results", 12
                ),
                "search_num_better_queries": yaml_data.get("search", {}).get(
                    "num_better_queries", 10
                ),
                # Fetching settings
                "max_concurrent_fetches": yaml_data.get("fetching", {}).get(
                    "max_concurrent_fetches", 20
                ),
                "per_domain_delay": yaml_data.get("fetching", {}).get(
                    "per_domain_delay", 0.8
                ),
                "fetch_timeout": yaml_data.get("fetching", {}).get("fetch_timeout", 30),
                "user_agent": yaml_data.get("fetching", {}).get(
                    "user_agent", "Mozilla/5.0..."
                ),
                "accept_encoding": yaml_data.get("fetching", {}).get(
                    "accept_encoding", "gzip, deflate, br"
                ),
                "max_content_chars": yaml_data.get("fetching", {}).get(
                    "max_content_chars", 8000
                ),
                # Filtering settings
                "min_relevance_score": yaml_data.get("filtering", {}).get(
                    "min_relevance_score", 3
                ),
                "disallowed_domains": yaml_data.get("filtering", {}).get(
                    "disallowed_domains", ["youtube.com", "youtu.be"]
                ),
                # Cache settings
                ...
                # Paths
                "prompts_path": Path(__file__).parent.parent
                / "prompts"
                / yaml_data.get("paths", {}).get("prompts_file", "prompts.yaml"),
                "config_path": config_path,
                # Logging
                "log_level": yaml_data.get("logging", {}).get("level", "INFO"),
                "log_format": yaml_data.get("logging", {}).get(
